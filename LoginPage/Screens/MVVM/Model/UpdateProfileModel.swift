...
struct ReferralCode: Codable {
    var status: Int?
    var message: String?
    var referralResult: ReferralResult?
    init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.status = try container.decodeIfPresent(Int.self, forKey: .status)
        self.message = try container.decodeIfPresent(String.self, forKey: .message)
        self.referralResult = try container.decodeIfPresent(ReferralResult.self, forKey: .referralResult)
    }
    enum CodingKeys: String, CodingKey {
        case status
        case message
        case referralResult = "result"
    }
    ...
    func encode(to encoder: any Encoder) throws {
        ...
        try container.encodeIfPresent(self.status, forKey: .status)
        try container.encodeIfPresent(self.message, forKey: .message)
        try container.encodeIfPresent(self.referralResult, forKey: .referralResult)
    }
}
